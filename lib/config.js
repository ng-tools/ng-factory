...
function ConfigRegistry(config) {
  ...
  if(false && config.pkg.ngfactory) {
    log('Using ng-factory config from %s', chalk.magenta(tildifyCwd('package.json', {cwd: config.cwd})));
    this.config = _.merge(config, checkConfig(config.pkg.ngfactory));
  } else {
    log('Using ng-factory config file %s', chalk.magenta(tildifyCwd('ngfactory.json', {cwd: config.cwd})));
...
function checkConfig(config, options) {
  ...
  if(options.file && config.ngVersion) {
    ...
    }, _.pick(config, 'username', 'sourcemaps'));
